...
@router.get("/dashboard/clients/{client_id}/action-history-debug")
def action_history_debug(request: Request, client_id: int):
    """
    Temporary coach-only diagnostic for historical commitment recovery.

    It intentionally returns only commitment/check-in/log metadata needed to
    reconstruct weekly action history. No private notes or health fields.
    """
    if not coach_is_logged_in(request):
        return RedirectResponse("/coach/login", status_code=303)
    ...
    def week_for(d):
        ...
        try:
            ...
            return None
        return max(1, (delta // 7) + 1) if delta >= 0 else 0
    ...
    with get_connection() as connection:
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                """
                SELECT id, call_date, created_at, updated_at
            ...
            cursor.execute(
                ...
                SELECT
                    action_id,
                    MIN(tracked_on) AS first_log_date,
                    MAX(tracked_on) AS last_log_date,
            ...
            log_summary = [dict(row) for row in cursor.fetchall()]
    ...
    for plan in plans:
        plan["plan_start_week"] = week_for(plan.get("start_date"))
        plan["plan_end_week"] = week_for(plan.get("end_date"))
        plan["linked_call_week"] = week_for(plan.get("linked_call_date"))
        log = logs_by_action.get(plan["id"])
        if log:
            plan["first_log_date"] = log.get("first_log_date")
            plan["last_log_date"] = log.get("last_log_date")
            plan["first_log_week"] = week_for(log.get("first_log_date"))
            plan["last_log_week"] = week_for(log.get("last_log_date"))
            plan["log_count"] = log.get("log_count")
            plan["completed_count"] = log.get("completed_count")
        else:
            plan["first_log_date"] = None
            plan["last_log_date"] = None
            plan["first_log_week"] = None
            plan["last_log_week"] = None
            plan["log_count"] = 0
