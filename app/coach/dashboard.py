...
def dashboard_home(request: Request):
    ...
    # Simplified coach dashboard model.
    # Keep all underlying signals, but only surface items that imply a clear
    # next action for Sushma.
    follow_up_clients = []
    ...
    for client in active_clients:
        ...
        # A client appears here only for missed tracking or low adherence.
        # Workouts behind, measurement due and no-next-call remain available in
        # the client workspace/list but do not make the main dashboard noisy.
        follow_up_reasons = []

        if (ops.get("missed_daily_count") or 0) >= 2:
            missed = ops.get("missed_daily_count") or 0
            follow_up_reasons.append(
                f"No tracking for {missed} day" + ("s" if missed != 1 else "")
            )
        ...
        # The existing weekly_review_overdue calculation is retained as the
        # source of truth, but presented to the coach as Ready for review.
